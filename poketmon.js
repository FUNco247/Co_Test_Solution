...
// 이때, 첫 번째(3번) 폰켓몬과 네 번째(3번) 폰켓몬을 선택하는 방법은 한 종류(3번 폰켓몬 두 마리)의 폰켓몬만 가질 수 있지만, 다른 방법들은 모두 두 종류의 폰켓몬을 가질 수 있다.
...
function solution(nums) {
  ...
  const pick = total / 2;
  const differentMonster = [...new Set(nums)].length;
  if (pick > differentMonster) {
    return differentMonster;
  } else {
    return pick;
  }
}
